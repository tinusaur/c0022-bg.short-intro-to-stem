...
- **Trio:** learner course **`C0102`**, this handbook **`C0202`**, slides **`C0602`**. Same lesson/topic spine as the EN trees. Educator tracks **`V21`** / **`V22`** are the teacher notes for learner **`V11`** / **`V12`**. There is no **`V23`** until `C0102` has a deeper learner track. Educators **show** `C0602` BG (learner-visible slides) while they follow this handbook.
...
- **Pairing:** each handbook track teaches the matching learner pages **and** the matching slides. Same `L####` / `T####` slugs as `C0102`. Do not add extra science the learner pages do not use. In delivered prose, name the groups in plain words (начинаещи читатели, по-млади читатели) — do not expose `Vxy` codes.
